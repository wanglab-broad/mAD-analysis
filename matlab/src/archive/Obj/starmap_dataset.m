classdef starmap_dataset
    %STARMAP_DATASET is the primary class object when running pipeline
    %   -----Properties-----
    %   input_path: path/to/sample/folder
    ...
    methods
        ...
        % Pipeline Obj
        function obj = STARMapDataset(input_path)
            obj.input_path = input_path;
            obj.out_path = fullfile(obj.input_path, 'output');
            fprintf('Pipeline Obj is generated...\n');
            
            if ~exist(obj.out_path, 'dir')
                mkdir(obj.out_path)
            end
        end
        
        % Load Raw Images
        function obj = LoadRawImages(obj)

            % load tiff image stacks
            obj.imgs = load_raw_imgs(obj.input_path);
        end
        
        % Histogram Equalization
        function obj = EqualizeHist3D(obj)
        ...
        function obj = LoadCodebook(obj, doReverse)
            ...
            [obj.nameToSeq, obj.seqToName] = LoadCodebook(obj.input_path, doReverse);            
        ...
        function obj = LoadNisslDapi(obj,zrange)
            ...
            nisslImagePath = fullfile(obj.input_path, 'nissl', [obj.prefix 'nissl_decon_ch01.tif']);
            dapiImagePath = fullfile(obj.input_path, 'nissl', [obj.prefix 'nissl_decon_ch00.tif']);
            if isempty(zrange)
            ...
            imwrite(obj.nissl, fullfile(obj.input_path, 'nissl', 'nissl_maxproj.tif'));
            imwrite(obj.dapi, fullfile(obj.input_path, 'nissl', 'dapi_maxproj.tif'));
        end
        ...
        function obj = AutomaticallySelectDapiCells(obj)
            ...
            save(fullfile(obj.input_path, 'output', 'cellLocs.mat'), 'cellLocs');
        end
