  ...
  for(i in levels(droplevels(meta_df[[highlight_col]]))){
    plot_list[[i]] <- ggplot() +
      ...
      geom_point(data = meta_df[meta_df[[highlight_col]] == i,], shape = 16, 
                 aes(x = UMAP1, y = UMAP2), colour = cell_type_palette_list[[title]][[which(cell_type_list[[title]] == i)]]) +#colour = pseudotime
...
ad_psdtime_gene_highlight <- function(meta_df,cds,gene_list){
  plot_list <- list()
  meta_df[["pseudotime"]] <- meta_df[["pseudotime"]]/max(meta_df[["pseudotime"]])
  for(i in gene_list){
    plot_list[[i]] <- ggplot() +
