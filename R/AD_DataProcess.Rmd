...
#AD_64 palette list
top_level_palette_list <- as.character(unlist(starmap_64@misc[["top_hex_dict_64"]]))
names(top_level_palette_list) <- names(unlist(starmap_64@misc[["top_hex_dict_64"]]))
top_level_palette_list <- c(top_level_palette_list[!names(top_level_palette_list) %in% c("DG","LHb")])
top_level_palette_list[["Mix"]] <- "#8c8c8c"
...
cell_type_palette_list <- list("Ex" = c("#f4777c","#f8bf76" ,"#fdfe02","#b0ff00"),
                    "Micro" = c("#2fe04d","#2c73e6","#fa733e"),
                    "Oligo" = c("#2fe04d","#2c73e6","#fa733e","#8600d4"),
                    "Astro" = c("#2fe04d","#2c73e6","#fa733e"),
                    "OPC" = c("#0dd193","#7a9deb"),
...
DE_13mo[["Micro"]] <- ad_D_vs_C_DE(starmap_scaled,"Micro","13_months",logfc_cutoff = 0,p_cutoff = 1)
DE_13mo[["Astro"]] <- ad_D_vs_C_DE(starmap_scaled,"Astro","13_months",logfc_cutoff = 0,p_cutoff = 1)
DE_13mo[["Oligo"]] <- ad_D_vs_C_DE(starmap_scaled,"Oligo","13_months",logfc_cutoff = 0,p_cutoff = 1)
DE_13mo[["Neuron"]] <-   ad_D_vs_C_DE(starmap_scaled,c("Ex","CA1","CA2","CA3","DG","Inhi"),"13_months",logfc_cutoff = 0,p_cutoff = 1)
DE_13mo[["DG"]] <- ad_D_vs_C_DE(starmap_scaled,"DG","13_months",logfc_cutoff = 0,p_cutoff = 1)

plot_list <- list()
plot_list[["Micro"]] <- ad_D_vs_C_DE_heatmap(starmap_scaled,"Micro","13_months",FC_cutoff = 0.1,draw_connector = T)
plot_list[["Astro"]] <- ad_D_vs_C_DE_heatmap(starmap_scaled,"Astro","13_months",FC_cutoff = 0.1,draw_connector = T)
plot_list[["Oligo"]] <- ad_D_vs_C_DE_heatmap(starmap_scaled,"Oligo","13_months",FC_cutoff = 0.1,draw_connector = T)  
plot_list[["Ex"]] <-   ad_D_vs_C_DE_heatmap(starmap_scaled,c("Ex","CA1","CA2","CA3","DG"),"13_months",FC_cutoff = 0.1,draw_connector = T)
plot_list[["Inhi"]] <-   ad_D_vs_C_DE_heatmap(starmap_scaled,c("Inhi"),"13_months",FC_cutoff = 0.1,draw_connector = T)
pdf(file = "F3-6_13mo_volcano_plot.pdf",width = 12,height = 9,useDingbats = FALSE)
plot_list %>% print()
dev.off()
...
for(i in c("Micro","Astro","Oligo","Neuron")){
  ...
  path = paste(i,"_13mo_DE.rnk",sep = ""),col_names = F)
}
...
DE_8mo[["Micro"]] <- ad_D_vs_C_DE(starmap_scaled,"Micro","8_months",logfc_cutoff = 0)
DE_8mo[["Astro"]] <- ad_D_vs_C_DE(starmap_scaled,"Astro","8_months",logfc_cutoff = 0)
DE_8mo[["Oligo"]] <- ad_D_vs_C_DE(starmap_scaled,"Oligo","8_months",logfc_cutoff = 0)
DE_8mo[["Neuron"]] <-   ad_D_vs_C_DE(starmap_scaled,c("Ex","CA1","CA2","CA3","DG","Inhi"),"8_months",logfc_cutoff = 0)
...
for(i in c("Micro","Astro","Oligo","Neuron")){
  ...
  path = paste(i,"_8mo_DE.rnk",sep = ""),col_names = F)
}
```
...
```{r GSEA}
#load 13mo GSEA result 
gsea_res <- list()
gsea_res[["Micro"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Micro_13mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Micro_13mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["Astro"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Astro_13mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Astro_13mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["Oligo"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Oligo_13mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Oligo_13mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05)%>% arrange(desc(NES))
gsea_res[["Neuron"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Neuron_13mo_pos.tsv"),
                              read_tsv(file = "Pathway_analysis/GSEA/Neuron_13mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["DG"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/DG_13mo_pos.tsv"),
                          read_tsv(file = "Pathway_analysis/GSEA/DG_13mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))

gsea_res[["Micro"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Micro_8mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Micro_8mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["Astro"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Astro_8mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Astro_8mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["Oligo"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Oligo_8mo_pos.tsv"),
                             read_tsv(file = "Pathway_analysis/GSEA/Oligo_8mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))
gsea_res[["Neuron"]] <- rbind(read_tsv(file = "Pathway_analysis/GSEA/Neuron_8mo_pos.tsv"),
                              read_tsv(file = "Pathway_analysis/GSEA/Neuron_8mo_neg.tsv")) %>% subset(`NOM p-val` < 0.05) %>% arrange(desc(NES))

myGO <- suppressMessages(GSA::GSA.read.gmt("Pathway_analysis/gprofiler_full_mmusculus.name.gmt"))
myGO <- data.frame(GOID = myGO$geneset.names, 
                   Description = myGO$geneset.descriptions, 
                   Term_size = lengths(myGO$genesets),
                   row.names = myGO$geneset.names)


#arrange gsea result
for(i in c("Micro","Astro","Oligo","Neuron","DG")){
  gsea_res[[i]] <- gsea_res[[i]] %>% .[,c(1,2,4:7)] %>% rename(`GS<br> follow link to MSigDB` = "GOID", `NOM p-val` = "p_val")
  gsea_res[[i]]$NAME <- myGO[gsea_res[[i]]$GOID,2] 
  gsea_res[[i]] <- gsea_res[[i]][myGO[gsea_res[[i]]$GOID,3] >= 5 & myGO[gsea_res[[i]]$GOID,3] <= 100, ] %>% arrange(p_val)
}

pdf(file = "F3-5_GSEA_Barplot_p_001.pdf",width = 8,height = 12,useDingbats = FALSE)
for(i in c("Micro","Astro","Oligo")){
plot_list[[i]] <- ggplot(data = gsea_res[[i]],
       aes(x = reorder(NAME,ES), y = ES, fill = ES < 0))+
...
top_GO <- gsea_res[["Micro"]]$NAME[1:9] %>% 
    union(gsea_res[["Astro"]]$NAME[1:3]) %>% 
    union(gsea_res[["Oligo"]]$NAME[1:4]) %>%
    union(gsea_res[["Neuron"]]$NAME[1:15])  
top_GO_df <- data.frame(name = top_GO,Micro = 0,Astro = 0,Oligo = 0,Neuron = 0)
for (i in 1:dim(top_GO_df)[1]) {
  for(j in c("Micro","Astro","Oligo","Neuron")){
    if(sum(as.character(gsea_res[[j]]$NAME) == top_GO_df$name[i]) == 0){
      top_GO_df[[j]][i] <- 0
    }else{
      top_GO_df[[j]][i] <- gsea_res[[j]]$NES[as.character(gsea_res[[j]]$NAME) == top_GO_df$name[i]]
    }
  }
}
require(gplots)
...
heatmap.2(as.matrix(as.data.frame(top_GO_df[,-1],row.names = as.character(top_GO_df$name))),
          Colv=F,
          dendrogram = "row",
          col = rev(colorRampPalette(brewer.pal(9, 'RdBu'))(128)),
          trace="none",
