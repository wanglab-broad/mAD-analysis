...
### image preprocessing
from skimage.morphology import ball, disk, opening, closing, dilation
...
opath = '/stanley/WangLab/Data/Processed/2022-01-03-Hu-AD_8m_Rep/output/clustermap-test/'

if not os.path.exists(opath):
    os.mkdir(opath)

current_tile_folder = os.path.join(opath, tile)
if not os.path.exists(current_tile_folder):
    os.mkdir(current_tile_folder)
...
### dapi pre-preprocessing
strel=ball(3)
filter_value = np.percentile(dapi[dapi != 0], 95)
dapi = np.where(dapi > filter_value, dapi, 0)
# Morphological closing
dapi = closing(dapi, strel)
# Dilation
dapi = dilation(dapi, strel)
# Median filter
for z in range(dapi.shape[0]):
    dapi[z, :, :] = median(dapi[z, :, :], disk(10))

dapi = dapi.astype(np.uint8)
tifffile.imwrite(os.path.join(current_tile_folder, 'pi_pp.tif'), dapi)
